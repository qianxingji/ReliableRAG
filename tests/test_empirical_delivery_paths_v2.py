...
from scripts.empirical_delivery_paths_v2 import BoundFiles, RootBinding, SourceLoader, bind_support, ORIGINAL_ROOT, SUPPORT_PATHS, relative_identity, windows_identity
...
class DeliveryPathTests(unittest.TestCase):
    ...
    def test_helper_free_module_preserves_absence_and_rejects_extra_helper(self):
        module = types.ModuleType("no_import_helper")
        module.ROOT = Path(ORIGINAL_ROOT)
        module.science = lambda x: x + 1
        science = module.science
        receipt = bind_support(module, self.physical, dict(ROOT=""), None)
        self.assertIs(module.science, science)
        self.assertNotIn("import_file", module.__dict__)
        self.assertFalse(receipt["import_file_replaced_by_authenticated_loader"])
        module.ROOT = Path(ORIGINAL_ROOT)
        module.import_file = lambda *args: None
        with self.assertRaises(ValueError): bind_support(module, self.physical, dict(ROOT=""), None)
        self.assertEqual(module.ROOT, Path(ORIGINAL_ROOT))
