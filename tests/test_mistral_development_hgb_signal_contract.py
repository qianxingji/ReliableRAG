...
from scripts.run_mistral_development_hgb_signal import (
    EXPECTED_TRACES,
    HGB_MODEL_SHA256,
    load_exact_hgb,
)
from scripts.validate_mistral_development_hgb_signal import (
    current_manifest_member_paths,
    independent_scores,
)


class MistralDevelopmentHGBSignalContractTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_independent_feature_formula_matches_native_on_invented_pair(self):
        ...
        trace = {
            ...
            "split": "development", "a0": "Alpha city", "a1": "Beta city",
    ...
    def test_validator_uses_independent_formula_and_not_producer(self):
        source = (Path(__file__).resolve().parents[1]
                  / "scripts/validate_mistral_development_hgb_signal.py").read_text(encoding="utf-8")
        ...
        self.assertNotIn("scripts.run_mistral_development_hgb_signal", imported_modules)
    ...
    def test_producer_requires_accepted_semantics_and_forbids_neural_runtime(self):
        source = (Path(__file__).resolve().parents[1]
                  / "scripts/run_mistral_development_hgb_signal.py").read_text(encoding="utf-8")
    ...
    def test_hgb_freeze_requires_exact_sources_model_and_control_graph(self):
        producer = (Path(__file__).resolve().parents[1]
                    / "scripts/run_mistral_development_hgb_signal.py").read_text(
                        encoding="utf-8"
                    )
        validator = (Path(__file__).resolve().parents[1]
                     / "scripts/validate_mistral_development_hgb_signal.py").read_text(
                         encoding="utf-8"
                     )
        self.assertIn("legacy_manifest_member_paths", producer)
        self.assertIn("UNIQUE_HGB_INPUTS", producer)
        self.assertIn("validate_mistral_development_hgb_signal.py", producer)
