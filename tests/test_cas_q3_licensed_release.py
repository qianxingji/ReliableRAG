...
from scripts.package_cas_q3_licensed_release import ARCHIVE_NAME, ROOT, build, scan_text as build_scan_text
from scripts.validate_cas_q3_licensed_release import CODE_PATHS, validate_archive, scan_text as validate_scan_text
...
class AggregateReleaseTests(unittest.TestCase):
    ...
    def test_valid_candidate_is_deterministic_and_roundtrips(self):
        ...
        self.assertEqual(result["status"], "PASS_CAS_Q3_APACHE2_AWARE_AGGREGATE_RELEASE_V2_ROUNDTRIP")
        self.assertFalse(result["distribution_authorized"])

    def test_license_scope_is_code_only_and_release_stays_blocked(self):
        with zipfile.ZipFile(self.archive) as bundle:
            manifest = json.loads(bundle.read("MANIFEST.json"))
            self.assertEqual(manifest["project_license"], "Apache-2.0")
            self.assertEqual(set(manifest["project_license_scope"]), CODE_PATHS)
            self.assertFalse(manifest["distribution_authorized"])
            self.assertTrue(manifest["non_code_members_not_relicensed_by_project_code_license"])
