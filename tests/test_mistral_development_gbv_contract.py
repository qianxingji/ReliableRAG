...
from scripts.run_mistral_development_gbv import (
    BATCH_SIZE,
    EXPECTED_TRACES,
    GBV_MODEL_ID,
    GBV_MODEL_REVISION,
...
from scripts.validate_mistral_development_gbv import (
    current_manifest_member_paths,
    entailment_probabilities,
)


class MistralDevelopmentGbVContractTests(unittest.TestCase):
    ...
    def test_branch_payload_recomputes_score_from_raw_logits(self):
        ...
        role, state, question, answer = "fit", "a0_e0", "q", "a"
        evidence = ["p"]
        pairs = [("p", "h"), ("p2", "h")]
            ...
            payload = {
                ...
                "forwards": [{
                    ...
                    "logits": {
                        ...
                        "dtype": "float32", "shape": [2, 2], "byte_order": sys.byteorder,
                    },
                }],
    ...
    def test_producer_gates_hgb_and_preserves_branch_order(self):
        source = (Path(__file__).resolve().parents[1]
                  / "scripts/run_mistral_development_gbv.py").read_text(encoding="utf-8")
        self.assertIn("PASS_INDEPENDENT_FORMULA_MISTRAL_DEVELOPMENT_HGB_SIGNAL", source)
    ...
    def test_unscorable_payload_rejects_hidden_hypothesis(self):
        trace = {"dataset": "d", "retriever": "bm25", "sample_id": "s", "position": 0}
        payload = {
            "status": "unscorable", **trace, "role": "fit", "state": "a0_e0",
            "question_sha256": hashlib.sha256(b'"q"').hexdigest(),
        ...
        with self.assertRaisesRegex(RuntimeError, "GBV_UNSCORABLE_PAYLOAD"):
            validate_branch_payload(
                payload, trace=trace, role="fit", state="a0_e0", question="q",
    ...
    def test_validator_is_tokenizer_logit_only_and_independent_of_producer(self):
        source = (Path(__file__).resolve().parents[1]
                  / "scripts/validate_mistral_development_gbv.py").read_text(encoding="utf-8")
        ...
        self.assertNotIn("scripts.run_mistral_development_gbv", imported_modules)
        ...
        self.assertIn("PASS_INDEPENDENT_TOKENIZER_LOGIT_MISTRAL_DEVELOPMENT_GBV", source)

    def test_gbv_freeze_requires_exact_sources_assets_and_control_graph(self):
        producer = (Path(__file__).resolve().parents[1]
                    / "scripts/run_mistral_development_gbv.py").read_text(
                        encoding="utf-8"
                    )
        validator = (Path(__file__).resolve().parents[1]
                     / "scripts/validate_mistral_development_gbv.py").read_text(
                         encoding="utf-8"
                     )
        self.assertIn("legacy_manifest_member_paths", producer)
        self.assertIn("UNIQUE_GBV_INPUTS", producer)
        self.assertIn("validate_mistral_development_gbv.py", producer)
        self.assertIn("NONEXACT_FROZEN_INPUT_GRAPH", validator)
        self.assertIn("GBV_PROVENANCE_PATH_COUNT", validator)
