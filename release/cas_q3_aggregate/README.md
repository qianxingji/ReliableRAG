# Paired RAG Repair: aggregate verification package

This package verifies the reporting layer for a fixed empirical study of
whether a repaired RAG answer should replace its original answer. It is an
anonymous, aggregate-only release candidate. It contains no author identity,
...
The project license is pending owner selection, so this candidate is not
authorized for public distribution. A project license cannot grant rights to
third-party datasets, model weights or dependencies; none of those payloads is
included here. See `THIRD_PARTY_NOTICES.md` for the upstream inventory and the
release action applied to each principal material.
