...
| 5. Historical manifest and receipt recovery | `P0_E_HISTORICAL_MANIFEST_ARCHIVE_RECOVERY.md`, its JSON/verifier receipts and Astra xhigh audit; `P0_E_ORIGINAL_FIT_RECEIPT_CENSUS.md` and its JSON receipt | One pre-ROA-named local package contains a complete `mars_full` manifest byte-identical to the current copy. A later bounded census scans 55,373 accessible files, 1,770 metadata candidates and 37 ZIPs, finding only the original seven model hashes plus their static-delivery copies and no receipt candidate. | An independently certified timestamp, original-fit authentication, any of the seven per-estimator ID/matrix receipts, an independent original-fit witness, or absence outside the scanned roots. |
...
author name and active corresponding email directly into the Git-ignored local
input; the tracked build-gate receipt records only the resulting counts. The
...
3. In a repository checkout, run:
   ...
   pinned SHA-256. Each accepted build reports 56 checks and recompiles to the
   exact accepted 12-page PDF hash.
...
5. Request private forensic materials only through a journal-approved channel.
   ...
   On the preserved Windows evidence host, the bounded receipt census can be
   repeated with `python scripts/audit_cas_q3_original_fit_receipt_census.py`.
