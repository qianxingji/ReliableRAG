...
| Current package fit | A provisional dry run raises the source to 12 pt, preserves numeric citations, flattens eight source members and compiles a font-clean 12-page PDF without scientific-content changes. The data/code statement now exposes the missing licensed persistent archive. It is not final: the Springer Nature template, single-anonymous identities, mandatory funding statement, cover letter and release review remain pending | Current 11-page main plus 3-page supplement is below 25 pages before template conversion; use `smallcondensed`, flatten upload files and recheck the final combined count | The current 155-word Applied Intelligence abstract exceeds this historical fallback's normal 150-word ceiling. A reproducible PDF proxy counts 3,978 tokens before References and 4,676 overall, below the published 5,000--7,500 average but not a stated hard minimum; Sage style, abstract revision, exact submission count and scope confirmation would be needed if this fallback were reactivated | Anonymous source and availability wording exist; current-title conversion and declarations remain, with a material novelty-fit risk |
