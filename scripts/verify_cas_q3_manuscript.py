...
def main() -> int:
    ...
    require("aggregate candidate remains withheld" in lower, "release-review boundary is explicit", checks)
    require("apache license 2.0 is present for project-authored code" in lower, "project-code license is explicit", checks)
    require("a corrected license-aware aggregate candidate has been built and validated" in lower, "corrected V2 archive state is explicit", checks)
    require("data and code availability" in lower, "data and code availability section is explicit", checks)
    require("no archive with a doi" in lower, "persistent code archive gap is explicit", checks)
    require("final submission revision and permanent archive identifier have not yet been frozen" in lower, "final code revision and permanent identifier remain open", checks)
    require("supplied if requested through a channel designated or accepted by the handling editor" in lower, "restricted evidence delivery is request-bound", checks)
