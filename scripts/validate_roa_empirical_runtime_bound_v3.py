"""V3 entry: accepted V2 bindings under the census-supported native default."""
import argparse
import hashlib
import json
import os
...
def prepare(config, pin):
    ...
    assert Path(controls["entry_v3"]["path"]).resolve()==Path(__file__).resolve()
    ...
    if "v3_fixture_acceptance" in controls:
        fixture=json.loads(payloads[controls["v3_fixture_acceptance"]["filename"]])
        assert fixture["status"]=="ACCEPTED_C3_VALIDATION_V3_INVENTED_ONLY"
...
def main():
    ...
    from scripts.empirical_validation_binding_v3 import V3ValidationBindings,observe_native_default
    environment_start=observe_native_default(frozen)
    ...
    binding=V3ValidationBindings(native,frozen,payloads,environment_start=environment_start,forbidden_events=events);binding.install()
