"""Independent tokenizer/logit validation of Mistral test paired GbV."""
...
def validate_branch(payload: dict, *, trace: dict, role: str, state: str,
    ...
    for batch_index, (forward, expected_tokens) in enumerate(
        ...
        require(path.is_relative_to(namespace)
                ...
                and meta["byte_order"] == "little", "LOGIT_BINDING")
...
def main() -> int:
    ...
    output = (args.output.resolve() if args.output else
              (root / "gbv_validation/VALIDATION.json").resolve())
    ...
    require(receipt["status"] == "PASS_MISTRAL_TEST_GBV_PENDING_INDEPENDENT"
    ...
    require(len(traces) == len(frozen_rows) == len(repair_rows)
            == EXPECTED_TRACES and len(a0_rows) == 36_000
            and len(a1_rows) == 90_000
            and len(a0_map) == len(repair_map) == len(a1_map)
    ...
    result = {
        "status": "PASS_INDEPENDENT_TOKENIZER_LOGIT_MISTRAL_TEST_GBV",
