"""Invented-only regression gate for the prospective C3 V2 binding."""
import argparse
import ast
import copy
import hashlib
...
def main():
    ...
    from scripts.empirical_validation_binding_v2 import (
        EXPECTED_CONSTANTS, V2ValidationBindings, constant_assignments,
        dependency_inventory, source_record)
    ...
    try:
        ...
        frozen = dict(tokenizer_source=cfg['tokenizer_source'], original_helper_asts=cfg['original_helper_asts'],
            ...
            freeze_sha256=args.config_sha256, embedded_controls=[])
        ...
        binding = V2ValidationBindings(fake_native(), frozen, {})
        ...
        assert invoke(V2ValidationBindings(fake_native(), changed, {}).load_functions, root)['reason'] == 'CONSTANT_ASSIGNMENT_AST_AND_VALUES'
        bad_pin = copy.deepcopy(frozen)
        bad_pin['constant_sources']['support']['sha256'] = '0' * 64
        assert invoke(V2ValidationBindings(fake_native(), bad_pin, {}).load_functions, root)['reason'] == 'FROZEN_CONSTANT_SOURCE_RECORDS'
        def prebound_loader(value):
            ...
            return loaded
        assert invoke(V2ValidationBindings(fake_native(loader=prebound_loader), frozen, {}).load_functions, root)['reason'] == 'EXACT_PREBIND_MISSING_GLOBALS'
        nested_scope = {}
        ...
        success.mkdir()
        control_bytes = b'{"scope":"invented V2 control"}\n'
        final_frozen = dict(frozen, embedded_controls=[dict(filename='INVENTED_V2_CONTROL.json',
            sha256=hashlib.sha256(control_bytes).hexdigest(), size_bytes=len(control_bytes))])
        final = fake_native(success)
        final_binding = V2ValidationBindings(final, final_frozen, {'INVENTED_V2_CONTROL.json': control_bytes},
            expected_generations=3, forbidden_events=events)
        ...
        failed_binding = V2ValidationBindings(failed_native, final_frozen, {'INVENTED_V2_CONTROL.json': control_bytes}, expected_generations=3)
        ...
        incomplete_binding = V2ValidationBindings(incomplete_native, final_frozen,
            {'INVENTED_V2_CONTROL.json': control_bytes}, expected_generations=3)
        ...
        result.update(status='PASS_C3_VALIDATION_V2_INVENTED_ONLY',
    ...
    return 0 if result['status'] == 'PASS_C3_VALIDATION_V2_INVENTED_ONLY' else 2
