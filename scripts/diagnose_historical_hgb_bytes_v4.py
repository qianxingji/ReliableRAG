...
def main():
    ...
    try:
        ...
        def encode(value,where):
            ...
            if callable(value):
                module_name=getattr(value,'__module__',None);qualified=getattr(value,'__qualname__',None)
                if isinstance(module_name,str) and isinstance(qualified,str) and module_name in sys.modules:
                    resolved=sys.modules[module_name]
                    for part in qualified.split('.'):
                        resolved=getattr(resolved,part,None)
                    if resolved is value:
                        return dict(global_reference=module_name+'.'+qualified)
            ...
            try:
                ...
                if not typ.__module__.startswith(('src.','sklearn.','numpy.')):
                    native=sys.modules.get('sklearn._loss._loss');receipt=cfg['native_loss_binary']
                    assert identity=='_loss.CyHalfBinomialLoss' and native is not None and typ is native.CyHalfBinomialLoss,('UNSUPPORTED_STATE_OBJECT',identity,where)
                    assert Path(native.__file__).resolve()==Path(receipt['path']).resolve() and io.sha(Path(native.__file__))==receipt['sha256']
                    result['native_loss_identity_binding']=dict(actual_reported_class=identity,exported_from='sklearn._loss._loss.CyHalfBinomialLoss',binary=receipt)
