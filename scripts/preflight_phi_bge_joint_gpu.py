...
OUT = REPO / "outputs/cas_q2/phi_bge_joint_gpu_preflight_v1"
...
def source_paths(original: Path) -> list[Path]:
    paths = [REPO / value for value in (
        "scripts/preflight_phi_bge_joint_gpu.py", "scripts/phi_bge_joint_preflight_guard.py",
...
def main() -> int:
    ...
    try:
        ...
        query = reader.generate_repair_query(key=key, question="In the invented scene, what color is the paper cog?",
        ...
        phase["stage"] = "a1"
        answer = reader.generate_answer(key=key, question="In the invented scene, what color is the paper cog?",
        ...
        result.update(status="PASS_PHI_BGE_JOINT_GPU_INVENTED_ONLY", admissions=admissions,
            short_generation={"capture": short_capture, "result": {"search_query": query.search_query, "parser_fallback": query.parser_fallback}},
