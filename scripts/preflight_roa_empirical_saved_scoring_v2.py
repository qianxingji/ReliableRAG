...
from scripts.empirical_scoring_io import REPO, inputs, saved_models, panel_scores, record, require
from scripts.empirical_scoring_import_v2 import native_scoring
from scripts.empirical_pool_io import verify_namespace, load, checked
...
def main():
    ...
    args=parser.parse_args();root=args.project_root.resolve();out=REPO/"outputs/cas_q2/empirical_saved_scoring_preflight_v2"
    ...
    try:
        ...
        failed=REPO/"outputs/cas_q2/empirical_saved_scoring_preflight_v1"
        diagnosis=REPO/"outputs/cas_q2/empirical_scoring_import_diagnostic_v1"
        paths += verify_namespace(failed,"d57d4d55849f32a981bae477fcbb1b98a84e9a42bdef705725dd56c2158a6fbd")
        paths += verify_namespace(diagnosis,"727b0741051fe971c084e006e8b904a47b28c7ea0db73df80212b776a3b04eb5")
        require(load(failed/"CPU_PREFLIGHT.json")["status"]=="FAIL","Original CPU failure preserved")
        diagnostic=load(diagnosis/"IMPORT_DIAGNOSTIC.json")
        require(diagnostic["status"]=="REPRODUCED_IMPORT_CACHE_MISS_DENIAL","Concrete import diagnostic required")
        e=diagnostic["source"];paths.append(checked(Path(e["path"]),e["sha256"],e["size_bytes"]))
        parent=root/"outputs/daa_v2_fresh_v1/prelabel_seal_v3"
        e=next(e for e in load(parent/"SHA256_MANIFEST.json")["files"] if e["path"].endswith("scoring/BASE_SCORE_PROVENANCE.json"))
        native_provenance=checked(root/e["path"],e["sha256"],e["size_bytes"]);paths.append(native_provenance)
        source_names=("scripts/empirical_scoring_io.py","scripts/empirical_scoring_guard.py","scripts/empirical_scoring_fixtures.py",
            "scripts/preflight_roa_empirical_saved_scoring.py","scripts/preflight_roa_empirical_saved_scoring_v2.py",
            "scripts/empirical_scoring_import_v2.py","tests/test_empirical_scoring_import_v2.py",
            "docs/cas_q2/EMPIRICAL_C4_IMPORT_CORRECTION.md","docs/cas_q2/EMPIRICAL_C4_SCORING_CONTRACT.md",
        ...
        require(native.source_definitions==load(native_provenance)["source_definitions"],"Native scientific AST records match original scoring run")
