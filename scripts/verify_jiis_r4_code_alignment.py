#!/usr/bin/env python3
"""Verify that the JIIS R4 PDF is mechanically sound and code-aligned."""
...
PDF = ROOT / "output/pdf/JIIS_Manuscript_R4_Code_Aligned.pdf"
R3 = ROOT / "paper/source_reference/JIIS_Manuscript_R3.pdf"
RECEIPT = ROOT / "paper/JIIS_R4_CODE_ALIGNMENT_VERIFICATION.json"
...
def main() -> int:
    ...
    require(PDF.is_file(), "code-aligned R4 PDF exists", checks)
    ...
    require(manifest["status"] == "PUBLIC_REPOSITORY_RELEASE", "public manifest records release status", checks)
    ...
    require(len(document) == 21, "R4 PDF has 21 pages", checks)
    require(all(abs(page.rect.width - 595.276) < 0.02 and abs(page.rect.height - 841.89) < 0.02 for page in document), "all R4 pages use A4 dimensions", checks)
    require(document.metadata.get("author") == "qianxingji", "R4 PDF author metadata matches", checks)
    require("code-aligned R4" in document.metadata.get("subject", ""), "R4 PDF subject records code alignment", checks)
    ...
    require(len(links) == 3, "R4 PDF contains three clickable public-repository links", checks)
    document.close()
    ...
    required_markers = [
        ...
        HGB_PATH,
        HGB_SHA256,
        "checks 129 reported statements",
        "not independent authentication of the original training event",
        "not independent reconstruction of the per-question top-K bootstrap",
    ]
    for marker in required_markers:
        require(marker in text, f"R4 text contains: {marker}", checks)
    stale_markers = [
    ...
    for marker in stale_markers:
        require(marker not in text, f"R4 omits stale release statement: {marker}", checks)
    ...
    require("Keep obtains 17.9889% EM and 22.7207% token F1" in text, "R4 reports the public Keep EM/F1 values", checks)
    require("352 Recoveries and 11 Damage events, giving Net = 341, EM = 19.8833%, and F1 = 25.2780%" in text, "R4 reports the public HGB+GbV_R counts and rates", checks)
    require("0.0611" in text, "R4 reports the public HGB+GbV_R Damage rate", checks)
    ...
    for marker in table_markers:
        require(marker in compact, f"R4 primary table contains {marker}", checks)

    font_rows = run("pdffonts", str(PDF)).splitlines()[2:]
    require(bool(font_rows), "R4 PDF contains font records", checks)
    require(all(row.split()[-5] == "yes" for row in font_rows), "all R4 fonts are embedded", checks)
    require(all("Type 3" not in row for row in font_rows), "R4 PDF contains no Type 3 fonts", checks)
    info = run("pdfinfo", str(PDF))
    require("Encrypted:       no" in info, "R4 PDF is not encrypted", checks)
    require("Form:            none" in info, "R4 PDF contains no form", checks)
    require("JavaScript:      no" in info, "R4 PDF contains no JavaScript", checks)

    result = {
        "schema_version": 1,
        "decision": "PASS_JIIS_R4_CODE_ALIGNMENT",
        "generated_utc": datetime.now(timezone.utc).isoformat(),
        "r3_source": {"path": str(R3.relative_to(ROOT)), "sha256": R3_SHA256},
        "r4_pdf": {"path": str(PDF.relative_to(ROOT)), "sha256": sha256(PDF), "pages": 21, "page_size": "A4"},
        "public_repository": {
            "url": PUBLIC_URL,
            "commit": PUBLIC_COMMIT,
            "github_actions_run": "https://github.com/qianxingji/ReliableRAG-Code/actions/runs/34993574899",
            "github_actions_conclusion": "success",
            "aggregate_reporting_checks": 129,
            "repository_checks": 167,
            "unit_tests": 7,
        },
        ...
        "known_boundary": "Public artifacts verify aggregate reporting and expose the current fitting/scoring procedures; excluded per-question and fitted assets prevent independent reconstruction of the selected top-K bootstrap and full neural acquisition.",
