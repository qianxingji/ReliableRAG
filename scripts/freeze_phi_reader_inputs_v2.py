"""V2 tokenizer-only Phi input freeze with recorded Python-environment reads."""
...
from scripts.phi_reader_input_freeze_common import (
    CONTEXT_BUDGET_CHARACTERS, DATASETS, MAXIMUM_GENERATION_INPUT_TOKENS,
...
OUT = REPO / "outputs/cas_q2/phi_reader_input_freeze_v2"
...
EXPECTED = {
    ...
    "preserved_v1_manifest": "c244000cb83b33d08f8a37419ee5c7aa6617f80311610d69706b3975e75dd2a9",
}
...
def input_paths(original: Path) -> list[Path]:
    ...
    paths = [dev / "runtime_branch_freeze/RUNTIME_CONFIG_FREEZE.json", dev / "runtime_branch_freeze/trace_manifest.jsonl",
        ...
        test / "phi_reader_input_freeze_v1/SHA256_MANIFEST.json",
        original / "prompts/baseline_v1.txt", original / "prompts/repair_missing_v1.txt",
        REPO / "scripts/phi_reader_input_freeze_common.py", REPO / "scripts/freeze_phi_reader_inputs_v2.py",
        REPO / "tests/test_phi_reader_input_freeze.py", REPO / "docs/cas_q2/PHI_READER_INPUT_FREEZE_CONTRACT.md",
        REPO / "docs/cas_q2/PHI_READER_INPUT_FREEZE_V2_AMENDMENT.md",
...
def sources(original: Path) -> tuple[list[dict], list[dict], dict]:
    ...
    require(digest(test / "phi_reader_input_freeze_v1/SHA256_MANIFEST.json") == EXPECTED["preserved_v1_manifest"], "preserved V1 failure pin")
...
def main() -> int:
    ...
    try:
        ...
        boundary = install_boundary(allowed, snapshot, OUT, (Path(sys.prefix), Path(sys.base_prefix)))
        ...
        environment_records = [record(Path(path)) for path in sorted(boundary["environment_reads"]) if Path(path).is_file()]
        ...
        result.update(status="PASS_VALUE_BLIND_PHI_INPUT_FREEZE_PENDING_INDEPENDENT", deterministic_prompt_rows=31500,
            ...
            batch_size=1, concurrent_reader_instances=1, execution_boundary=compact_boundary,
            environment_files_actually_read=len(environment_records), preserved_v1_failure=True)
    except Exception as exc:
        result.update(error_type=type(exc).__name__, diagnostic=str(exc))
        if boundary is not None:
            result["execution_boundary"] = {key: (len(value) if key == "environment_reads" else value) for key, value in boundary.items()}
