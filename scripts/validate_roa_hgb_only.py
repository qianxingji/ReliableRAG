"""Independent full reconstruction of the supplementary HGB-only control; no executor or fit imports."""
...
CONTROL = {"HGB_ONLY_R": [0]}
SINGLE_COMPARATORS = ["HGB_ONLY_R","GBV_ONLY_R"]
PREVIOUS = REPO / "outputs/cas_q2/supervision_matched_controls_v1"
METHODS = ["ROA-FULL","ROA-NOGBV","HGB","GbV","V2","GBV_ONLY_R","HGB_GBV_R",*CONTROL]
...
def main():
    ...
    verify_controls(PREVIOUS)
    c=bind_original(root,out)
    ind=load_original("independent");ind.METHODS=METHODS;ind.VAR=["ROA-FULL","HGB_GBV_R"]
    if any(n in sys.modules for n in ("design","learning","metrics","execute","src.arbitration.recovery_hgb_control","scripts.run_roa_hgb_only")):
    ...
    try:
        ...
        jobs={r["job_id"]:r for r in manifest["jobs"]};eq(len(jobs),28)
        calls={(r["job_id"],r["stage"]):r for r in manifest["calls"]};eq(len(calls),56)
        events=list(c.lines(out/"MODEL_FIT_CALLS_PRIVATE.jsonl"));eq(len(events),112)
        eq([r for r in events if r["event"]=="fit_completed"],manifest["calls"])
        eq(manifest["scientific_fit_calls"],56)
        ...
        with threadpool_limits(1):
            for item in split["contexts"]:
                ...
                for name in METHODS[:7]:
                    if name in ("ROA-FULL","ROA-NOGBV","GBV_ONLY_R","HGB_GBV_R"):
                        saved=c.keyed((PREVIOUS if name in ("GBV_ONLY_R","HGB_GBV_R") else c.OUT)/"jobs"/(context["id"]+"_"+name)/"PREDICTIONS_PRIVATE.jsonl")
                ...
                for name in CONTROL:
                    ...
                    for ds in ind.DS:
                        for retr in ind.RET:
                            n=sum(k[:2]==(ds,retr) for k in select["HGB_GBV_R"])
                ...
                for name in METHODS[:7]:second[name]=select[name]
        ...
        eq(model_count,28);eq(prediction_count,19212);eq(len(contexts),28)
        ...
        wins={m:sum(r["pooled"]["methods"]["HGB_GBV_R"]["net"]>r["pooled"]["methods"][m]["net"] and r["pooled"]["methods"]["HGB_GBV_R"]["damage"]<=r["pooled"]["methods"][m]["damage"] for r in reps) for m in SINGLE_COMPARATORS}
        med={m:statistics.median(r["pooled"]["comparisons"]["HGB_GBV_R"][m]["delta_em_pp"] for r in reps) for m in SINGLE_COMPARATORS}
        transport={m:{r["heldout"]:r["summary"]["comparisons"]["HGB_GBV_R"][m]["delta_em_pp"] for r in lodo["transports"]} for m in SINGLE_COMPARATORS}
        support=all(wins[m]>=4 and med[m]>0 and all(v>=-.10 for v in transport[m].values()) for m in SINGLE_COMPARATORS)
        eq(load("DEVELOPMENT_DECISION.json"),dict(decision="TWO_SIGNAL_INCREMENT_SUPPORTED_FOR_DESIGN_REVIEW" if support else "TWO_SIGNAL_INCREMENT_NOT_ESTABLISHED",successes=wins,median_fusion_minus_single_em_pp=med,lodo_fusion_minus_single_em_pp=transport,development_only=True,final_candidate_selected=False,new_confirmation_started=False))
        receipt=load("SCIENTIFIC_EXECUTION_RECEIPT.json");eq(receipt["status"],"SCIENTIFIC_EXECUTION_COMPLETE_PENDING_INDEPENDENT");eq(receipt["scientific_fit_calls"],56);eq(receipt["started_calls"],56);eq(receipt["boundary"]["blocked"],[])
        c.parents();c.frozen();verify_controls(PREVIOUS)
        for record in c.load(root/spec["manifest_path"])["files"]:c.check(record)
        state.update(status="PASS",checks=ind.CHECKS,max_numeric_error=ind.MAX_ERROR,contexts=28,model_bundles=28,prediction_rows=19212,aggregate_prediction_rows=81000,aggregate_action_rows=81000,
                     action_membership_differences=0,historical_control_fit_calls_verified=56,no_executor_or_fitter_import=True,original_artifacts_unchanged=True,validated_utc=c.now())
    ...
    if state["status"]=="PASS":
        ...
        write_json(out/"SEAL.json",dict(status="PASS",sealed_utc=c.now(),files=payload,scientific_fit_calls=56,independent_fit_calls=0,decision=load("DEVELOPMENT_DECISION.json")["decision"],hard_stop=True,cas_q2_status="NOT READY"))
