"""Execute exactly the two prospectively frozen supervision-matched controls.
...
CONTROL_FIELDS = {"GBV_ONLY_R": [10], "HGB_GBV_R": [0, 10]}
REFERENCES = ["ROA-FULL", "ROA-NOGBV", "HGB", "GbV", "V2"]
METHODS = REFERENCES + list(CONTROL_FIELDS)
NAMESPACE = "outputs/cas_q2/supervision_matched_controls_v1"


def attribution(repetitions, transports):
    successes, medians, lodo = {}, {}, {}
    for m in CONTROL_FIELDS:
        successes[m] = sum(r["pooled"]["methods"]["ROA-FULL"]["net"] > r["pooled"]["methods"][m]["net"] and
                           r["pooled"]["methods"]["ROA-FULL"]["damage"] <= r["pooled"]["methods"][m]["damage"] for r in repetitions)
        medians[m] = statistics.median(r["pooled"]["comparisons"]["ROA-FULL"][m]["delta_em_pp"] for r in repetitions)
        lodo[m] = {r["heldout"]:r["summary"]["comparisons"]["ROA-FULL"][m]["delta_em_pp"] for r in transports}
    support = all(successes[m] >= 4 and medians[m] > 0 and min(lodo[m].values()) >= -.10 for m in CONTROL_FIELDS)
    return dict(decision="COMPLEXITY_SUPPORTED_FOR_CONFIRMATION" if support else "SIMPLER_CONTROL_COMPETITIVE_REVIEW_CONTRIBUTION",
                successes=successes, median_full_minus_control_em_pp=medians, lodo_full_minus_control_em_pp=lodo,
...
def main():
    ...
    from src.arbitration.recovery_controls import fit_control
    threadpool_info()
    ...
    try:
        ...
        source_files = ["scripts/run_roa_controls.py","scripts/validate_roa_controls.py","scripts/replay_roa_original.py","src/arbitration/recovery_controls.py",
                        "docs/cas_q2/CONTROLS_PROTOCOL.md","docs/cas_q2/P0_1_CLIENT_ACCEPTANCE.md"]
        ...
        freeze = dict(frozen_utc=c.now(),commit=commit,command=sys.argv,
                      ...
                      expected_model_bundles=56,expected_fit_calls=112,scientific_fit_calls_before_freeze=0,
                      controls=CONTROL_FIELDS,synthetic_fit_calls=0)
        ...
        c.write("SCIENTIFIC_EXECUTION_STARTED.json",dict(started_utc=c.now(),expected_calls=112))
            ...
            for stage in ("primary","lodo"):
                ...
                for item in split["contexts"]:
                    ...
                    for m in REFERENCES:
                        ...
                        if m in ("ROA-FULL","ROA-NOGBV"):
                            scores[m] = {k:v["pR"] for k,v in c.keyed(c.OUT/"jobs"/(context["id"]+"_"+m)/"PREDICTIONS_PRIVATE.jsonl").items()}
                    ...
                    for m in CONTROL_FIELDS:
                        ...
                        for ds in d.HELDOUT:
                            for retriever in d.RETRIEVERS:
                                cap = sum(k[:2]==(ds,retriever) for k in selections["ROA-FULL"])
                ...
                if stage=="primary":
                    ...
                    c.write("ALLOCATION_SENSITIVITY.json",dict(repetitions=secondary_reps,folds=[dict(context=r["context"],summary=r["allocation_sensitivity"],breakdown=r["allocation_breakdown"]) for r in results],label="SECONDARY_ONLY; exact ROA-FULL stratum counts per partition"))
        ...
        if len(jobs)!=56 or len(calls)!=112 or state["started_calls"]!=112: raise RuntimeError("FIT_COUNT")
        c.write("MODEL_FIT_MANIFEST.json",dict(jobs=jobs,calls=calls,base_fit_calls=56,platt_fit_calls=56,scientific_fit_calls=112))
        ...
        state.update(status="SCIENTIFIC_EXECUTION_COMPLETE_PENDING_INDEPENDENT",model_bundles=56,completed_utc=c.now(),retrieval_generation_calls=0,
        ...
        lines=["# Supervision-matched development controls", "", "CAS Q2 STATUS: NOT READY.", "", "Decision: "+decision["decision"], "", "| Seed | ROA Net / Damage | GBV_ONLY_R Net / Damage | HGB_GBV_R Net / Damage |", "|---|---|---|---|"]
        for r in reps:
            mm=r["pooled"]["methods"];lines.append("| "+str(r["seed"])+" | "+" | ".join(str(mm[m]["net"])+" / "+str(mm[m]["damage"]) for m in ("ROA-FULL","GBV_ONLY_R","HGB_GBV_R"))+" |")
