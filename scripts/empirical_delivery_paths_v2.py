"""Version 2: original import-helper presence is preserved. Source transport; never used by the live controllers.
...
def bind_support(module, physical_root, relative_paths, importer):
    ...
    if importer is None:
        require("import_file" not in before, "Unexpected import utility on helper-free support")
    else:
    ...
    changed = set(changes) | ({"import_file"} if importer is not None else set())
    ...
    return dict(bindings={name: dict(logical=str(before[name]), physical=str(value)) for name, value in changes.items()},
                unchanged_scientific_function_objects=True, import_file_replaced_by_authenticated_loader=importer is not None)
...
class SourceLoader:
    ...
    def load(self, name, physical_path):
        ...
        if support is not None:
            ...
            importer = None if relative == "outputs/daa_v2_fresh_v1/retrieval_freeze/retrieval_support.py" else self.load
            receipt.update(bind_support(module, self.files.roots[entry["root"]].physical, bindings, importer))
