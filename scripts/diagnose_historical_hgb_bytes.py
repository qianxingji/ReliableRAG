    ...
    try:
        ...
        def encode(value,where):
            ...
            try:
                ...
                if callable(getter):observed=getter();mechanism='__getstate__'
                elif hasattr(value,'__dict__'):observed=dict(value.__dict__);mechanism='__dict__'
                else:raise RuntimeError(('UNSUPPORTED_STATE_OBJECT',identity,where))
                stats['objects']=stats.get('objects',0)+1
                return dict(kind='object',class_name=identity,mechanism=mechanism,state=encode(observed,where+'/state'))
