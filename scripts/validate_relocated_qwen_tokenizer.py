"""Separate original validator selections; no producer/native renderer import."""
import argparse
import collections
import hashlib
import json
...
def main():
    ...
    try:
            ...
            for mode, ordered in [('canonical', traces), ('replay', replay)]:
                ...
                try:
                    for i, trace in enumerate(ordered, 1):
                        ...
                        strata[k[:2]] += 1
                        if i % 1500 == 0: print('INDEPENDENT_QWEN_TOKENIZER', mode, i, len(ordered), flush=True)
        ...
        result.update(status='PASS_FULL_HISTORICAL_QWEN_TOKENIZER_ONLY', coverage=coverage, generation_receipts=41040,
            ...
            scope='Historical input/decoder validation only; shared fixed tokenizer implementation; no retrieval or neural replay')
