"""Freeze common Mistral development features before any Gold access."""
...
def validate_predecessors(root: Path) -> tuple[Path, Path, Path, Path]:
    ...
    require(
        ...
        and hgb_result["producer_receipt_sha256"]
        == sha256(hgb / "STAGE_RECEIPT.json")
        and hgb_result["hgb_signal_rows_sha256"]
        == sha256(hgb / "HGB_SIGNAL_ROWS.jsonl")
    ...
    require(
        ...
        and gbv_result["producer_receipt_sha256"]
        == sha256(gbv / "STAGE_RECEIPT.json")
...
def main() -> int:
    ...
    result: dict[str, object] = {
        ...
        "stage": "prelabel", "gold_values_read": 0, "scientific_fits": 0,
        "test_rows_read": 0, "neural_model_loads": 0, "model_forwards": 0,
        "hgb_model_loads": 0,
    }
    try:
        ...
        freeze = {
            ...
            "scope": "development feature rows only; neural models/Gold/test/fit forbidden",
        }
        ...
        rows = []; forced = collections.Counter(); role_counts = collections.Counter()
        eligible_role_counts = collections.Counter()
        for position, (trace, frozen, hgb_row, gbv_row) in enumerate(zip(
                traces, development, hgb_rows, gbv_rows, strict=True)):
            require(trace["position"] == frozen["position"] == position
                    ...
                    and hgb_row["role"] == frozen["role"],
                    "PRELABEL_FROZEN_IDENTITY")
            row = merge_prelabel_row(hgb_row, gbv_row)
            rows.append(row); role_counts[row["role"]] += 1
            if row["eligible"]:
                eligible_role_counts[row["role"]] += 1
            else:
                forced[row["forced_keep_reason"]] += 1
        require(role_counts == {"fit": 10_800, "cal": 2_700},
                "PRELABEL_ROLE_COUNTS")
        ...
        result.update(
            status="PASS_MISTRAL_DEVELOPMENT_PRELABEL_PENDING_INDEPENDENT",
        ...
        result.update(
            status="FAIL_MISTRAL_DEVELOPMENT_PRELABEL",
