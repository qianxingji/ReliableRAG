...
\begin{abstract}
Retrieval repair can recover an incorrect answer but can also replace a correct answer. We study post-generation selection between original and repaired answers under shared candidates, matched supervision, and a fixed action allocation. The evaluation uses one Qwen2.5-3B-Instruct reader, 6,000 questions from three multi-hop QA datasets, three retrieval conditions, and nine policies. This design makes useful recoveries and harmful replacements visible under one intervention budget. A calibrated logistic policy combining a frozen internal HGB score with a paired Generate but Verify adaptation improves exact match by 0.2333 percentage points and reduces full-population Damage by 0.0722 points relative to the supervision-matched GbV-only policy; multiplicity-adjusted question-cluster bootstrap ranges are [0.0722, 0.4333] and [-0.1444, -0.0278]. The same joint rule does not pass against the HGB-only policy, and the 25-feature ROA-FULL policy does not advance over the two-signal policy. The result supports a bounded within-reader empirical attribution rather than a new selector architecture, reader-general transfer, or deployment-cost claim.
\end{abstract}

\noindent\textbf{Keywords:} retrieval-augmented generation; answer verification; selective prediction; reliability; empirical evaluation
...
HGB abbreviates histogram-based gradient boosting and denotes a frozen historical pairwise selector, not a model fitted on the current 4,500-question development cohort. Let $L_{ij}$ be the reader's mean teacher-forced log probability for answer $a_i$ under evidence $E_j$. The selector receives 48 state-symmetric features: eight repaired-minus-original branch differences and their interactions with five pair-context variables. Its historical binary target is one for an incorrect $a_0$ followed by a correct $a_1$ and zero for the reverse transition; pairs in which both answers have the same EM status do not enter that fit. Training augments every $(x,y)$ with $(-x,1-y)$, and the saved HGB probabilities for $x$ and $-x$ are symmetrized at scoring time. A larger HGB score therefore means stronger support for choosing $a_1$. The complete feature dictionary, formula, fixed fit parameters, training scope, and provenance boundary appear in the supplementary section ``Frozen historical HGB score.''
...
\end{itemize}
Raw HGB, raw GbV, and V2 have different upstream training or rule dependencies and serve as context rather than matched attribution controls. Raw HGB ranks by the frozen historical score directly. HGB-only$_R$ instead fits a current Recovery head using that one score, its missingness flag, and retriever indicators; HGB+GbV$_R$ adds the paired GbV margin and its missingness flag. Both current heads are ordinary calibrated logistic regression, and neither refits the historical HGB estimator.
...
\resizebox{\textwidth}{!}{\input{tables/main_results.tex}}
\end{table*}
...
\label{tab:primary}
\resizebox{\textwidth}{!}{\input{tables/primary_comparisons.tex}}
\end{table*}
...
The numerical analysis is backed by hash-pinned aggregate files and independent implementations. A deterministic anonymous aggregate package contains the complete point estimates, intervals, claim summary, code/data availability statement, and verification scripts. Its historical candidate archive hash is \nolinkurl{b785890366995c2943007f5635e622814bc7961dc7b3ded4adee0707dfb7ca8d}. That immutable candidate remains withheld because its notice predates the corrected third-party terms review. A corrected Apache-2.0-aware candidate has been rebuilt and independently validated; its release still requires institutional review, final authorization, and persistent archiving.
...
The selected cohort has no dataset/ID overlap with a known 18,600-question exclusion union, but ID separation is not semantic decontamination and cannot rule out model pretraining exposure or unknown unlogged historical use. Benchmark-supplied contexts may also share dataset-specific structure with answers despite the field-restricted pool construction. Seven historical upstream estimators are byte-fixed and their saved-parameter computation is replayed. A locally preserved pre-ROA-named package contains a byte-identical copy of the complete historical training manifest, but no external record independently certifies its timestamp; the original per-estimator fit-time ID/matrix receipts and an independent original-fit witness remain missing. Reproduction therefore authenticates the saved computation within checked environments, not a literal replay of every original training event. The supplement records the distinct historical and current development populations.
...
An anonymous, aggregate-only verification package has been built and independently validated. It contains no questions, answers, Gold strings, per-example outcomes, model weights, local paths, or identities. Apache License 2.0 is present for project-authored code, and a corrected license-aware aggregate candidate has been built and validated. The project code is held in a public GitHub repository, but the final submission revision and permanent archive identifier have not yet been frozen. Public distribution of the aggregate candidate remains withheld pending confirmation of the legal holder/year, institutional and third-party release review, final archive authorization, and a persistent identifier. No archive with a DOI or other persistent identifier has been released. The final submission will provide a frozen code link and immutable archive identifier after the owner and institutional release gates pass. Benchmark- or model-restricted evidence will be described with its access conditions and supplied if requested through a channel designated or accepted by the handling editor, subject to upstream terms. The manuscript claims aggregate verification, not end-to-end public neural reproduction.
