"""Fixed empirical-panel fitter derived from the frozen control implementation.

Only the variant-width map and test-to-probe output role are changed. The
original fit-only transformation, base fit and disjoint Platt fit are retained.
Probe is not held out and provides no performance estimate.
"""
import hashlib
import warnings
import numpy as np
...
def fit_panel(rows, training_outcomes, parts, variant, job_id, event):
    """Fit/calibrate once; probe is overlapping opened data for replay only."""
    ...
    width = {'ROA-FULL': 11, 'ROA-NOGBV': 10, 'HGB_GBV_R': 2, 'HGB_ONLY_R': 1, 'GBV_ONLY_R': 1}[variant]
    ...
    raw = xx['probe']@base.coef_[0]+base.intercept_[0]
    ...
    predictions = {k:dict(logit_R=float(raw[i]),pR=float(prob[i])) for i,k in enumerate(kk['probe'])}
